...
def main_proc(args):
    subject, video_file, v, in_dir_base, base_path = args
    ...
    num_frames = len(vr)  # total frame count
    plot_frames = list(range(0, num_frames, 100))
    batch = vr.get_batch(plot_frames).asnumpy()  # (K, H, W, 3), RGB
    frames = {idx: frame for idx, frame in zip(plot_frames, batch)}
    del vr

    output = []
    result_files = list(in_dir.glob("*.pickle.gz"))
...
if __name__ == "__main__":
    root_dir = pathlib.Path(r"\\et-nas.humlab.lu.se\Gábor\pupil_size_validation\analysis_set\recordings")
    base_path = pathlib.Path(r"\\et-nas.humlab.lu.se\Gábor\pupil_size_validation\analysis_set\segmentation")
    ...
    for subject in subject_folders:
        ...
        for v in vids:
            ...
            if out_file.is_file():
                continue
            to_run.append( (subject, video_file, v, in_dir_base, base_path) )

    # run in parallel
    with pebble.ProcessPool(max_workers=4) as pool:
        future = pool.map(main_proc, to_run)
