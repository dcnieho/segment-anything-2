...
root_dir = pathlib.Path(r"D:\nvgaze\nvgaze_real_dataset_vr")
gt_dir   = pathlib.Path(r"D:\nvgaze\nvgaze_real_dataset_vr\TEyeDS\ANNOTATIONS")
base_path = pathlib.Path(f"//et-nas.humlab.lu.se/FLEX/datasets real/nvgaze_real_dataset_vr/sam2/persubject_run/")
do_plot_mask = False
def process(video):
    in_dir = base_path / video.name
    in_gt = gt_dir / f'NVIDIAVR_{video.name.lstrip("0")}_1.mp4pupil_seg_2D.mp4'
    ...
    scores = evaluate_segments(video.name, result_files, in_gt, save_csv_dir=out_dir, save_csv_fname=f'{video.name}.csv', ori_stim_fold=root_dir/video.name if do_plot_mask else None)
...
if __name__ == "__main__":
    subject_folders = [f for f in root_dir.iterdir() if f.is_dir() and not 'TEyeDS' in f.name]
