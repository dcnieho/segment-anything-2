...
def _pick_frame_index_from_valid(valid_pupil: pd.Series,
                                 ...
                                 lid_gt: pd.DataFrame,
                                 frame_idx_override: int | None,
                                 eyelid_area_low_frac: float = 0.40,
                                 eyelid_area_high_frac: float = 0.60) -> int:
    """
    Choose a GT row index (frame) to prompt by running BOTH:
      - New eyelid-area gating logic:
          1) Compute mean eyelid polygon area over all valid eyelid frames.
          2) If frame 0 has area >= low_frac * mean_area and combined validity -> choose 0;
             else scan forward to the first frame with area >= high_frac * mean_area and combined validity.
      - Original validity-based logic:
          - If frame 0 is combined-valid -> choose 0;
          - Else find the first long run (>10) of combined-valid frames and pick index 4 in that run;
          - Else fallback to first combined-valid frame, or 0 if none.

    Returns the LATER (max) of the two selected indices.

    Args:
        valid_pupil: pd.Series of booleans for pupil validity (CENTER X != -1).
        valid_iris:  pd.Series of booleans for iris validity  (CENTER X != -1).
        lid_gt: Eyelid landmarks DataFrame (same row indexing as GT tables).
        frame_idx_override: Optional explicit GT row index to use.
        eyelid_area_low_frac: Threshold fraction for accepting frame 0 (default 0.40).
        eyelid_area_high_frac: Threshold fraction for forward scan acceptance (default 0.60).

    Returns:
        int: chosen GT row index (later of the two strategies).
    """
    if frame_idx_override is not None:
        return int(frame_idx_override)

    # Align lengths defensively
    n = min(len(valid_pupil), len(valid_iris), len(lid_gt))
    if n == 0:
        return 0

    valid_combined = (valid_pupil.iloc[:n].reset_index(drop=True).to_numpy()
                      & valid_iris.iloc[:n].reset_index(drop=True).to_numpy())

    # ---------- New eyelid-area gating ----------
    def _polygon_area(poly_xy: np.ndarray) -> float:
        """Shoelace formula; returns 0.0 for degenerate polygons."""
    ...
    eyelid_valid = np.zeros(n, dtype=bool)
    for i in range(n):
        poly = _eyelid_polygon_from_row(lid_gt.iloc[i])
        A = _polygon_area(poly)
        areas[i] = A
        eyelid_valid[i] = (A > 0.0 and len(poly) >= 3)

    new_idx = None
    if np.any(eyelid_valid):
        mean_area = float(np.mean(areas[eyelid_valid]))
        low_thr = float(eyelid_area_low_frac) * mean_area
        high_thr = float(eyelid_area_high_frac) * mean_area

        # First-frame check
        if valid_combined[0] and eyelid_valid[0] and areas[0] >= low_thr:
            new_idx = 0
        else:
            # Scan forward
            for i in range(1, n):
                if eyelid_valid[i] and valid_combined[i] and areas[i] >= high_thr:
                    new_idx = int(i)
                    break

    # ---------- Original validity-based selection ----------
    old_idx = None
    valid_series = pd.Series(valid_combined)

    if valid_series.iloc[0]:
        old_idx = 0
    else:
        a = (valid_series.diff(1) != 0).astype('int').cumsum()
        a[~valid_series] = -1
        b = a.groupby(a).size()

        # First long run (>10), excluding invalid group (-1)
        candidates = np.where(np.logical_and(b > 10, b.index != -1))[0]
        if len(candidates) > 0:
            long_enough = candidates[0]
            fr_idxs = np.where(a == b.index[long_enough])[0]
            old_idx = int(fr_idxs[4] if len(fr_idxs) > 4 else fr_idxs[0])
        else:
            idxs = np.where(valid_series.values)[0]
            old_idx = int(idxs[0]) if len(idxs) > 0 else 0

    # ---------- Return the LATER of the two ----------
    candidates = []
    if new_idx is not None:
        candidates.append(int(new_idx))
    if old_idx is not None:
        candidates.append(int(old_idx))
...
def _polygon_area(poly_xy: np.ndarray) -> float:
    ...
    Returns 0.0 for degenerate polygons (<3 vertices).
    """
    if poly_xy is None or len(poly_xy) < 3:
        return 0.0
    x = poly_xy[:, 0]
    y = poly_xy[:, 1]
    # roll by -1 to close the ring
    return 0.5 * abs(np.sum(x * np.roll(y, -1) - np.roll(x, -1) * y))
...
def retrieve_prompt_from_subject(video_path,
    ...
    fr_idx = _pick_frame_index_from_valid(
        valid_pupil,
        valid_iris,
        lid_gt,
        override_idx,
        eyelid_area_low_frac=0.5,
        eyelid_area_high_frac=0.75
    )
    # if fr_idx==0:
    #     return
...
if __name__ == '__main__':
    ...
    prompt_out_dir = pathlib.Path(r"\\et-nas.humlab.lu.se\FLEX\datasets real\TEyeD\prompts3")

    prompt_frame_hardcode = {'DikablisSA_1_2.mp4': 70,
                             'DikablisSA_6_2.mp4': 170,
                             'DikablisSA_10_2.mp4': 130,
                             'DikablisSA_13_1.mp4': 10,
                             'DikablisSA_16_1.mp4': 190,
                             'DikablisSA_16_2.mp4': 10,
                             'DikablisSA_20_2.mp4': 80,
                             'DikablisSA_22_1.mp4': 160,
                             'DikablisSA_24_1.mp4': 90,
                             'DikablisSA_25_1.mp4': 10,
                             'DikablisSA_26_1.mp4': 90,
                             'DikablisSA_27_2.mp4': 40,
                             'DikablisSA_28_1.mp4': 10,
                             'DikablisSA_35_1.mp4': 30,
                             'DikablisSA_37_1.mp4': 190,
                             'DikablisSS_1_1.mp4': 10,
                             'DikablisSS_3_1.mp4': 20,
                             'DikablisSS_8_1.mp4': 10,
                             'DikablisSS_13_1.mp4': 10,
                             'DikablisSS_17_1.mp4': 10,
                             'DikablisSS_17_2.mp4': 10,
                             'DikablisSS_24_1.mp4': 10,
                             'DikablisSS_25_1.mp4': 90,
                             'DikablisSS_28_1.mp4': 10,
                             'DikablisSS_30_1.mp4': 10,
                             'DikablisSS_31_1.mp4': 10,
                             'DikablisSS_38_1.mp4': 10,
                             'DikablisT_1_4.mp4': 10,
                             'DikablisT_1_7.mp4': 10,
                             'DikablisT_2_6.mp4': 10,
                             'DikablisT_6_2.mp4': 70,
                             'DikablisT_7_1.mp4': 10,
                             'DikablisT_7_3.mp4': 110,
                             'DikablisT_8_2.mp4': 40,
                             'DikablisT_8_3.mp4': 10,
                             'DikablisT_8_11.mp4': 20,
                             'DikablisT_9_9.mp4': 10,
                             'DikablisT_12_1.mp4': 10,
                             'DikablisT_12_11.mp4': 10,
                             'DikablisT_13_6.mp4': 50,
                             'DikablisT_14_1.mp4': 20,
                             'DikablisT_14_7.mp4': 10,
                             'DikablisT_16_1.mp4': 50,
                             'DikablisT_16_3.mp4': 10,
                             'DikablisT_16_4.mp4': 10,
                             'DikablisT_18_7.mp4': 20,
                             'DikablisT_18_8.mp4': 20,
                             'DikablisT_19_4.mp4': 20,
                             'DikablisT_21_1.mp4': 20,
                             'DikablisT_24_1.mp4': 10,
                             'DikablisT_24_2.mp4': 10,
                             'DikablisT_25_2.mp4': 20,
                             'DikablisT_25_7.mp4': 20,
                             'DikablisT_26_3.mp4': 10,
                             'DikablisT_26_4.mp4': 10,
                             'DikablisT_27_4.mp4': 10,
                             'DikablisT_31_3.mp4': 40,
                             'DikablisT_31_13.mp4': 40,
                             'dikablisR_1_2.mp4': 120,
                             'GW_5_4.mp4': 120,
                             'GW_5_7.mp4': 10, 
                             'GW_6_6.mp4': 12,
                             'GW_7_7.mp4': 20,
                             ...
                             'LPW_17_1.mp4': 150,
                             'LPW_22_1.mp4': 10,
                             'NVIDIAVR_1_1.mp4': 300,
                             'NVIDIAVR_14_1.mp4': 90}
    ...
    for d in datasets:
        ...
        for v in video_files:
            if not v.name.startswith('NVIDIA'):
                continue
