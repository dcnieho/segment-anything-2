...
def propagate(predictor, inference_state, chunk_size, save_path=None, prompts=None, save_range=None, stop_early=False):
    ...
    if prompts is not None and prompts['frame']>0:
        ...
        for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state, reverse=True):
            ...
            if save_path and (out_frame_idx==prompts['frame'] or (save_range and out_frame_idx in save_range)):
...
def load_prompts_from_folder(folder: pathlib.Path, file_name: str):
    ...
    with open(folder / (file_name+"_prompts.txt")) as f:
        reader = csv.reader(f, delimiter="\t")
        pr = list(reader)
    # get associated frame
    prompt_imgs = list(folder.glob(file_name+"_*.png"))
    if len(prompt_imgs)!=1:
        raise ValueError('wrong')
    prompt_img = extract_last_number(prompt_imgs[0].name)

    prompts[prompt_imgs[0]] = {'frame':None, 0:{'coords':[], 'labels':[]}, 1:{'coords':[], 'labels':[]}}
    for i,p in enumerate(pr):
        obj_id = 0 if p[0]=='beam' else 1 if p[0]=='pupil' else 2
        ...
        prompts[prompt_imgs[0]][obj_id]['coords'].append(point_coord)
        prompts[prompt_imgs[0]][obj_id]['labels'].append(label)
    return prompts


if __name__ == '__main__':
    input_dir   = pathlib.Path(r"\\et-nas.humlab.lu.se\Gábor\pupil_size_validation\pilot7_251022")
    prompts_base = pathlib.Path(r"\\et-nas.humlab.lu.se\Gábor\pupil_size_validation\pilot7_251022\sam2\prompts")
    output_base  = pathlib.Path(r"\\et-nas.humlab.lu.se\Gábor\pupil_size_validation\pilot7_251022\sam2\segmentation")
    ...
    for subject in subject_folders:
        ...
        for i,video_file in enumerate(video_files):
            if not video_file.exists():
                continue
            if not (prompts_base / subject.name / (video_file.name+"_prompts.txt")).exists():
                continue
            try:
                ...
                for i,video_segments in enumerate(propagate(predictor, inference_state, chunk_size, this_output_path, prompts, save_range=range(0,1200,10), stop_early=False)):
