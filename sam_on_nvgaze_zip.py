...
def retrieve_prompt_from_subject(zip_path, frame_idx):
    print(f'Retrieving prompt from {zip_path}' )

    with zipfile.ZipFile(zip_path, 'r') as zipf:
        with zipf.open(f"type_maskWithoutSkin_frame_{frame_idx:04d}.png")  as eyeball_file:
            img = np.array(Image.open(eyeball_file).convert('RGB'))
            mask_true = (img[:, :, 1] > 150) & (img[:, :, 0] < 100) & (img[:, :, 2] < 100)

        with zipf.open(f"type_maskWithSkin_frame_{frame_idx:04d}.png")  as skin_file:
            img = np.array(Image.open(skin_file).convert('RGB'))
            skin_mask = (img[:, :, 0] > 150) & (img[:, :, 1] < 100) & (img[:, :, 2] < 100)

    # get the green part (pupil)
    pupil_mask = mask_true & ~skin_mask
    pupil_ys, pupil_xs = np.where(pupil_mask)
    pupil_x = np.mean(pupil_xs)
    pupil_y = np.mean(pupil_ys)

    return {
        'prompt': {
            'pupil': {
                'points': np.array([[pupil_x, pupil_y]]),
                'labels': np.array([1]),
                'box': None
            },
            'iris': {
...
if __name__ == '__main__':
    ...
    for video_dir in subject_folders:
        try:
            ...
            this_prompt = retrieve_prompt_from_subject(video_dir, frame_idx)
