...
def retrieve_prompt_from_subject(file_name, gt):
    ...
    return {
        'prompt': {
            'pupil': {
                'points': np.array([gt['p_x'].iloc[i].to_numpy(), gt['p_y'].iloc[i].to_numpy()]).reshape(-1,2),
                'labels': np.array([1]),
                'box': None
            },
            'iris': {
...
if __name__ == '__main__':
    ...
    output_bin = pathlib.Path(f"//et-nas.humlab.lu.se/FLEX/datasets synthetic/OpenSFEDS/sam2/{from_sample}/") # will contain saved masks
    ...
    gt  = pd.read_csv(r"D:\OpenSFEDS\Static\prompts\prompts.tsv", delimiter='\t')
    ...
    predictor = build_sam2_video_predictor("sam2_hiera_l.yaml", r"C:\Users\Dee\Desktop\sam2\sam2-dee\checkpoints\sam2_hiera_large.pt", device=device)
